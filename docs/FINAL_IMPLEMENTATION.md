...
- реализована desktop-часть — daemon с PTY-мостом и прозрачными shell-shims (срез разработчика 1):
  ...
  - desktop распространяется через Homebrew third-party tap (`brew tap alaunert/cucoudle && brew install cucoudle`): формула собирает virtualenv в libexec, устанавливает пакет вместе с runtime-зависимостями (pydantic/websockets/qrcode) через bootstrapped pip и регистрирует `brew services`-демон; тест формулы импортирует daemon-модуль, подтверждая наличие зависимостей;
