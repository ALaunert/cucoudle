...
- реализован канал десктоп↔мобила — shared-протокол и relay-брокер (срез разработчика 3):
  ...
  - production relay пишет структурные JSON audit logs для connection lifecycle и protocol routing: role, desktop/mobile/session/request IDs, method/event, byte counts и result; тестовый флаг `RELAY_LOG_INPUT_TEXT=true` дополнительно пишет `inputText` для mobile input и `outputText` для PTY `terminal.output`, при этом pairing codes и tokens не журналируются;
