...
- реализован технически проверенный двухфазный брендированный запуск мобильного приложения: до готовности JavaScript Expo показывает нативный splash с локальным PNG на фоне `#07111E` в режиме `contain`, затем index route без искусственной задержки показывает React Native экран на том же фоне с тем же PNG и отдельными wordmark `Cucoudle`, tagline `AI CODING AGENTS · ONE CHAT` и доступным индикатором загрузки, пока bootstrap выбирает начальный маршрут; Expo public config разрешает asset и все три splash-параметра, а React-композиция покрыта component test, но визуальный переход фаз не проверен;
...
- реализованы продуктовые экраны Action Inbox и Sessions: status-derived attention cards, exact-key dismissal, generic lifecycle activity, offline/reconnecting states, active/completed filters, cwd basename и доступная навигация к session detail; сессии в статусе `waiting` закрепляются вверху общего и активного списков, а их строки выделены attention-цветами и явным статусом «Ждёт вашего ответа»; во «Входящих» сессия открывается тапом по всему телу attention card и по строке recent activity (кроме `removed`-событий, у которых сессии уже нет), а «Скрыть» и structured actions не вызывают навигацию;
...
- реализованы live Session detail, New и Settings: моноширинный terminal buffer, цельный composer с multiline-полем и встроенной 44pt кнопкой-стрелкой, отправка текста с `submit: true` и преобразованием в PTY Enter (`\r`) на desktop, connection/recovery controls, подключение другого компьютера и очистка pairing profile; отдельная destructive-кнопка interrupt удалена из session UI, экран учитывает системную клавиатуру — на iOS keyboard-aware контейнер поднимает composer, на Android окно работает в режиме `resize`, а оба варианта flex-терминала могут сжиматься до доступной высоты; нативное окно Expo и корневой navigator используют общий фон `#07111E`, чтобы он не просвечивал белым за скруглёнными углами системной клавиатуры;
...
- реализована desktop-часть — daemon с PTY-мостом и прозрачными shell-shims (срез разработчика 1):
  ...
  - серверный рендер терминала: демон прогоняет копию PTY-байтов через эмулятор pyte (`TerminalRenderer`) и шлёт мобиле коалесцированные (50 мс) кадры `terminal.render`; private DSR и Kitty keyboard queries фильтруются только в render-копии, а любая ошибка renderer отключает styled render для сессии, но не может прервать raw PTY I/O; mobile `StyledTerminal` имеет readable presentation parser (очистка invisible chars, компактные positional gaps, divider rows и схлопывание blank rows), точный режим `1:1` с горизонтальным скроллом, ANSI-стили и возврат к live tail; при отсутствии render-кадров остаётся fallback на `PlainTerminal`;
...
Репозиторий содержит описание продукта, проектные спецификации, рабочий production relay, desktop-daemon с PTY-мостом и shell-shims, а также Expo SDK 54 workspace с проверенной Wave 3 application composition: live terminal/session controls, structured approval gating и reconnect/recovery lifecycle. Технически настроены две разные splash-фазы на фоне `#07111E`: native до готовности JavaScript показывает локальный PNG в режиме `contain`, затем index route показывает тот же PNG и отдельные брендовые тексты с индикатором только на время фактического bootstrap без таймера или анимации. Мобильный composer — единое поле со встроенной стрелкой отправки; он передаёт чистый текст с `submit: true`, а desktop доставляет его как настоящий терминальный ввод: bracketed paste (когда TUI включил режим) плюс отдельный Enter (`\r`) через 60 мс, поэтому ответ не только вставляется, но и подтверждается в интерактивном CLI; тот же механизм используется и текстовым ответом на interaction-карточку (`interaction.respond`), так что сообщение с телефона больше не «проглатывается» как незавершённый paste. Отдельная кнопка «Прервать» удалена из экрана как визуально конкурирующее destructive-действие; backend interrupt contract не удалялся. Экран открытой сессии обёрнут в полноэкранный `KeyboardAvoidingView`: iOS использует padding-avoidance, Android — системный window resize, поэтому шапка и composer сохраняют естественную высоту, а `PlainTerminal` и `StyledTerminal` больше не удерживают прежний минимум 180 px и сжимаются в доступное пространство. Нативный фон Expo и `contentStyle` корневого Stack теперь совпадают с тёмным фоном приложения `#07111E`; это закрывает белый underlying window, который был виден в верхних скруглениях iOS-клавиатуры. Независимо разработанные desktop и relay соединены и проверены **воспроизводимым кросс-язык harness** (`npm run test:integration`, `tests/integration/desktop-relay-smoke.ts`): настоящий Python-демон против запущенного TS relay + mobile WebSocket-клиент проходят `register`/`pairing`/`session.list`/спаун сессии/`subscribe`/`session.input`→`terminal.output`. Тот же smoke проходит и против production relay (`RELAY_WS=wss://relay.launert.dev`), в том числе на macOS. Последний общий mobile gate зелёный: 28/28 suites, 179/179 tests, mobile typecheck и Expo Doctor 18/18; ранее подтверждены 58 core tests и 81 desktop pytest. Mobile runtime composition из Task 14 собран (`createMobileRuntime` + application-тест `mobileFlow`). Проверены config resolution и React component contract, но визуальный переход native → React, масштаб и отсутствие вспышки на release-сборке и физическом iPhone/Android не подтверждены; точная анимация и inset системной клавиатуры, включая отсутствие белых углов после исправления, также требуют повторного device-smoke, как и полный сквозной runtime smoke именно через Expo-приложение; настоящий Claude/Codex/Cursor в интеграционном прогоне заменялся контролируемым `/bin/cat` за shim `claude`.
...
- нативная и React-фазы splash технически настроены и проверены через config resolution/component test; они используют общий фон и PNG, но React отдельно добавляет wordmark, tagline и progress indicator. Визуальный переход, масштаб и отсутствие вспышки не подтверждены: startup smoke release-сборки на физическом iPhone/Android ещё не выполнен; запуск через Expo Go также не подтверждён фактическим прогоном на устройстве.
