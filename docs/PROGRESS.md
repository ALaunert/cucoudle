...
## 2026-07-11 — Убран белый фон в скруглениях iOS-клавиатуры

**Цель:** Устранить две белые области, просвечивавшие по бокам верхних скруглений системной клавиатуры на экране открытой сессии.

**Сделано:** Диагностика снимка и конфигурации показала, что тёмный `#07111E` был задан splash и React-экранам, но не underlying native window и не корневому navigator. В Expo-конфигурацию добавлен общий `backgroundColor: #07111E`, а корневой Stack получил тот же `contentStyle.backgroundColor`. Добавлен regression test, фиксирующий совпадение обоих уровней с цветом темы.

**Затронутые компоненты:** `apps/mobile/app.json`, `apps/mobile/src/app/_layout.tsx`, `apps/mobile/src/ui/__tests__/NativeBackground.test.ts`, `docs/PROGRESS.md`, `docs/FINAL_IMPLEMENTATION.md`.

**Проверки:** focused regression test — 1/1; после rebase поверх параллельных terminal-изменений полный mobile Jest — 28/28 suites, 179/179 tests; TypeScript typecheck — успешно; Expo Doctor — 18/18; resolved public Expo config подтверждает `backgroundColor=#07111E`.

**Решения, ограничения и проблемы:** Цвет задан и на нативном, и на navigator-уровне, потому что скруглённая клавиатура может открывать underlying surface вне текущего React-экрана. Автотест и Expo config resolution подтверждают контракт, но результат после пересборки ещё не проверен повторным снимком на физическом iPhone. В общем Jest-прогоне сохраняются два известных предупреждения Expo Router о лишних mock routes `new` и `settings`.

**Следующий шаг:** Перезапустить или пересобрать мобильное приложение, открыть сессию на iPhone, сфокусировать composer и подтвердить, что оба верхних угла клавиатуры теперь заполнены тёмным фоном без белых полос.
